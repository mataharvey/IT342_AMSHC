...
export class Command {
   ...
   * and the script being run in argv[1], with appUser parameters after that.
   *
   * @example
   * ```
   * program.parse(process.argv);
   * program.parse(); // implicitly use process.argv and auto-detect node vs electron conventions
   * program.parse(my-args, { from: 'appUser' }); // just appUser supplied arguments, nothing special about argv[0]
   * ```
   *
   * @returns `this` command for chaining
   */
   ...
   * program.parseAsync(my-args, { from: 'appUser' }); // just appUser supplied arguments, nothing special about argv[0]
   * ```
   *
   * @returns Promise
   */
