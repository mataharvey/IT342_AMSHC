...
<p align="center">
  <b>Lightweight, beautiful and appUser-friendly interactive prompts</b><br />
...
Array of [prompt objects](#-prompt-objects).
 These are the questions the appUser will be prompted. You can see the list of supported [prompt types here](#-types).
...
Its signature is `(prompt, answer, answers)` where `prompt` is the current prompt object, `answer` the appUser answer to the current question and `answers` the appUser answers so far. Async functions are supported.
...
Callback that's invoked when the appUser cancels/exits the prompt.
Its signature is `(prompt, answers)` where `prompt` is the current prompt object and `answers` the appUser answers so far. Async functions are supported.
...
If a value is an instance of `Error` it will simulate the appUser cancelling/exiting the prompt.
...
Each property be of type `function` and will be invoked right before prompting the appUser.
...
The message to be displayed to the appUser.
...
Receive the appUser input and return the formatted value to be used inside the program.
...
| format | `function` | Receive appUser input. The returned value will be added to the response object |
| validate | `function` | Receive appUser input. Should return `true` if the value is valid, and an error message `String` otherwise. If `false` is returned, a default error message is shown |
...
> Prompts appUser for invisible text input.
...
> Prompts appUser for number input.
...
| format | `function` | Receive appUser input. The returned value will be added to the response object |
| hint | `string` | Hint to display to the appUser |
...
The prompt will list options based on appUser input. Type to filter the list.
