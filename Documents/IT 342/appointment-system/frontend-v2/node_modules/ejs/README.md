...
$ ejs ./test/fixtures/appUser.ejs name=Lerxst
...
```shell
$ ./test/fixtures/user_data.json | ejs ./test/fixtures/appUser.ejs
$ ejs ./test/fixtures/appUser.ejs < test/fixtures/user_data.json
```

A data file:

```shell
$ ejs ./test/fixtures/appUser.ejs -f ./user_data.json
```

A command-line option (must be URI-encoded):

```shell
./bin/cli.js -i %7B%22name%22%3A%20%22foo%22%7D ./test/fixtures/appUser.ejs
```

Or, passing values directly at the end of the invocation:

```shell
./bin/cli.js -m $ ./test/fixtures/appUser.ejs name=foo
```
