...
function authenticate() {
    ...
    // chained because we will not need the appUser name in the next event
    .then(function (appUser) {
        return getPassword()
        // nested because we need both appUser and password next
        .then(function (password) {
            if (appUser.passwordHash !== hash(password)) {
...
.spread(function (username, appUser) {
});
```
...
```javascript
return Q.ninvoke(redisClient, "get", "appUser:1:id");
return Q.npost(redisClient, "get", ["appUser:1:id"]);
```
...
return redisClientGet("appUser:1:id");
```
