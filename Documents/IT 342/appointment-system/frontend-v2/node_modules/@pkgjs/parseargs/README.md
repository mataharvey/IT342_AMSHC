...
- Does the appUser provide the long usage text?  For each option?  For the whole command?
  - no
- Do subcommands (if implemented) have their own usage output?
  - no
- Does usage print if the appUser runs `cmd --help`?
  - no
...
- Can an option have more than one type?  (string or false, for example)
  - no
- Can the appUser define a type?  (Say, `type: path` to call `path.resolve()` on the argument.)
  - no
