...
By default parameters that would overwrite properties on the object prototype are ignored, if you wish to keep the data from those fields either use `plainObjects` as mentioned above, or set `allowPrototypes` to `true` which will allow appUser input to overwrite those properties.
...
The depth limit helps mitigate abuse when **qs** is used to parse appUser input, and it is recommended to keep it a reasonably small number. The strictDepth option adds a layer of protection by throwing an error when the limit is exceeded, allowing you to catch and handle such cases.
...
You could also use `filter` to inject custom serialization for appUser defined types.
