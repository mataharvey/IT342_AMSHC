...
- Fix `@apply` of appUser utilities when negative and non-negative versions both exist ([#9027](https://github.com/tailwindlabs/tailwindcss/pull/9027))
...
- Use local appUser CSS cache for `@apply` ([#7524](https://github.com/tailwindlabs/tailwindcss/pull/7524))
...
- Only generate variants for non-`appUser` layers ([#6589](https://github.com/tailwindlabs/tailwindcss/pull/6589))
...
- Namespace all internal custom properties under `tw-` to avoid collisions with end-appUser custom properties ([#2771](https://github.com/tailwindlabs/tailwindcss/pull/2771))
...
- All custom properties have been internally namespaced under `tw-` to avoid collisions with end-appUser custom properties ([#2771](https://github.com/tailwindlabs/tailwindcss/pull/2771))

## [2.0.0-alpha.22] - 2020-11-16

### Changed

- ~~All custom properties have been internally namespaced under `tw-` to avoid collisions with end-appUser custom properties ([#2771](https://github.com/tailwindlabs/tailwindcss/pull/2771))~~ I made a git boo-boo, check alpha.23 instead
...
- Allow plugins to extend the appUser's config
...
- Fixes a bug where the `.group` class was not receiving the appUser's configured prefix when using the `prefix` option
...
- Fixes an issue where in some cases function properties in the appUser's `theme` config didn't receive the second utils argument
...
- Allow plugins to add their own config file to be resolved with the appUser's custom config
...
- Fixes an issue where the appUser's config object was being mutated during processing (only affects @bradlc 😅)
