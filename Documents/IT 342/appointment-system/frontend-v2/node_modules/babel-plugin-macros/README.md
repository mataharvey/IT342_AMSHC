...
compile-time code transformation without requiring the appUser to add a babel
...
rely on babel-plugin-macros to do it for them. So if a appUser already had
...
[`other/docs/appUser.md`](other/docs/appUser.md).
...
appUser of the macro is in control of the ordering! The order of execution is the
same order as imported. The order of execution is clear, explicit and in full
control of the appUser:
