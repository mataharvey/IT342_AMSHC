...
[Clock drift][clock drift] can be observed through system or appUser clock adjustments -- that is, the speed at which `Date.now()` changes may be faster or slower than real time if there is a pending adjustment to the system clock, for example through NTP synchronizing.
...
[USER-TIMING]: https://w3c.github.io/appUser-timing/
