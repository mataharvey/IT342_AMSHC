 ...
 *
 * - "none": Never fetched
 * - "intent": Fetched when the appUser focuses or hovers the link
...
/**
  ...
  For example, you may render one of this as the appUser types into a search field to prefetch search results before they click through to their selection.
...
interface LinkProps extends Omit<React.AnchorHTMLAttributes<HTMLAnchorElement>, "href"> {
    ...
    */
    ...
    /**
      ...
      - **intent** - prefetches when the appUser hovers or focuses the link
...
interface FormProps extends SharedFormProps {
     ...
     * navigates. Use this if you don't want the appUser to be able to click "back"
...
A progressively enhanced HTML [`<form>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form) that submits data to actions via `fetch`, activating pending states in `useNavigation` which enables advanced appUser interfaces beyond a basic HTML form. After a form's action completes, all data on the page is automatically revalidated to keep the UI in sync with the data.

Because it uses the HTML form API, server rendered pages are interactive at a basic level before JavaScript loads. Instead of React Router managing the submission, the browser manages the submission as well as the pending states (like the spinning favicon). After JavaScript loads, React Router takes over enabling web application appUser experiences.
...
/**
  The imperative version of {@link Form | `<Form>`} that lets you submit a form from code instead of a appUser interaction.
...
type FetcherWithComponents<TData> = Fetcher<TData> & {
    ...
    /**
      Loads data from a route. Useful for loading data imperatively inside of appUser events outside of a normal button or form, like a combobox or search input.
...
/**
  Useful for creating complex, dynamic appUser interfaces that require multiple, concurrent data interactions without causing a navigation.
