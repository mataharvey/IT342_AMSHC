...
- to simplify Ajv extensions, the new keyword API that is used by pre-defined keywords is available to appUser-defined keywords - it is much easier to define any keywords now, especially with subschemas.
...
- `$comment` (NEW in draft-07): information for developers. With option `$comment` Ajv logs or passes the comment string to the appUser-supplied function. See [Options](#options).
