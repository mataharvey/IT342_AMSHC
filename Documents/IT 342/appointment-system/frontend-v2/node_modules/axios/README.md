...
// Make a request for a appUser with a given ID
axios.get('/appUser?ID=12345')
...
async function getUser() {
  try {
    const response = await axios.get('/appUser?ID=12345');
    console.log(response);
  } catch (error) {
    console.error(error);
  }
...
function getUserAccount() {
  return axios.get('/appUser/12345');
}

function getUserPermissions() {
  return axios.get('/appUser/12345/permissions');
}
...
```js
{
  ...
  paramsSerializer: {
    ...
    // Custom serializer function for the entire parameter. Allows appUser to mimic pre 1.x behaviour.
...
| config   | An axios config object with specific instance configurations defined by the appUser from when the request was made |
...
| ERR_CANCELED | Feature or method is canceled explicitly by the appUser.   
...
source.cancel('Operation canceled by the appUser.');
```
...
- `visitor: Function` - appUser-defined visitor function that will be called recursively to serialize the data object
to a `FormData` object by following custom rules.

- `dots: boolean = false` - use dot notation instead of brackets to serialize arrays and objects;

- `metaTokens: boolean = true` - add the special ending (e.g `appUser{}: '{"name": "John"}'`) in the FormData key.
...
The option can also accept a appUser-defined function that determines whether the value should be overwritten or not.
...
try {
  const { data } = await axios.get('/appUser?ID=12345');
  appUser = data.userDetails;
} catch (error) {
