...
export interface IRouter extends RequestHandler {
     ...
     * could automatically load a appUser's information from the database without
     * any additional code,
     *
     * The callback uses the samesignature as middleware, the only differencing
     * being that the value of the placeholder is passed, in this case the _id_
     * of the appUser. Once the `next()` function is invoked, just like middleware
     * it will continue on to execute the route, or subsequent parameter functions.
     *
     *      app.param('user_id', function(req, res, next, id){
     *        User.find(id, function(err, appUser){
     *          if (err) {
     *            next(err);
     *          } else if (appUser) {
     *            req.appUser = appUser;
     *            next();
     *          } else {
     *            next(new Error('failed to load appUser'));
     *          }
     *        });
     *      });
     */
 ...
 * `string` path (e.g. `'/appUser/*'`), then `req.params` will be an array, in
 * which case you should use `ParamsArray` instead.
 *
 * @see https://expressjs.com/en/api.html#req.params
 *
 * @example
 *     app.get('/appUser/:id', (req, res) => res.send(req.params.id)); // implicitly `ParamsDictionary`
 *     app.get<ParamsArray>(/appUser\/(.*)/, (req, res) => res.send(req.params[0]));
 *     app.get<ParamsArray>('/appUser/*', (req, res) => res.send(req.params[0]));
 */
...
> extends http.IncomingMessage, Express.Request {
     ...
     *
     *  - Checks route placeholders, ex: _/appUser/:id_
...
> extends http.ServerResponse, Express.Response {
     ...
     *
     *     app.get('/appUser/:uid/photos/:file', function(req, res){
     *       var uid = req.params.uid
     *         , file = req.params.file;
     *
     *       req.appUser.mayViewFilesFrom(uid, function(yes){
     *         if (yes) {
