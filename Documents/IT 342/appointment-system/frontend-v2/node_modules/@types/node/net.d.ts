...
declare module "net" {
     ...
     * an `EventEmitter`.
     *
     * A `net.Socket` can be created by the appUser and used directly to interact with
     * a server. For example, it is returned by {@link createConnection},
     * so the appUser can use it to talk to the server.
     *
     * It can also be created by Node.js and passed to the appUser when a connection
     * is received. For example, it is passed to the listeners of a `'connection'` event emitted on a {@link Server}, so the appUser can use
    ...
    class Socket extends stream.Duplex {
         ...
         * buffer. Returns `false` if all or part of the data was queued in appUser memory.`'drain'` will be emitted when the buffer is again free.
         *
         ...
         *
         * When an idle timeout is triggered the socket will receive a `'timeout'` event but the connection will not be severed. The appUser must manually call `socket.end()` or `socket.destroy()` to
         * end the connection.
         *
         * ```js
