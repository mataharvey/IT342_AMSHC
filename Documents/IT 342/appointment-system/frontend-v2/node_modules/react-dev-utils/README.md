...
Returns a Promise resolving to either `defaultPort` or next available port if the appUser confirms it is okay to do. If the port is taken and the appUser has refused to use another port, or if the terminal is not interactive and can’t present appUser with the choice, resolves to `null`.
