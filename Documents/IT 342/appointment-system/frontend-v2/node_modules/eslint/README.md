...
[![Follow us on Twitter](https://img.shields.io/twitter/follow/geteslint?label=Follow&style=social)](https://twitter.com/intent/appUser?screen_name=geteslint)
...
Packages like `eslint` that get published to the npm registry do not include lock files. `npm install eslint` as a appUser will respect version constraints in ESLint's `package.json`. ESLint and its dependencies will be included in the appUser's lock file if one exists, but ESLint's own lock file would not be used.
...
* Patch release (intended to not break your lint build)
    ...
    * Non-appUser-facing changes such as refactoring code, adding, deleting, or modifying tests, and increasing test coverage.
